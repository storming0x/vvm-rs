...
/// The string describing the [vvm::Platform] to build for
///
...
/// - "macosx-aarch64"
/// - "windows-amd64"
pub const VVM_TARGET_PLATFORM: &str = "VVM_TARGET_PLATFORM";

/// Returns the platform to generate the constants for
///
/// if the `VVM_TARGET_PLATFORM` var is set, this will return the matching [vvm::Platform], otherwise the native platform will be used [vvm::platform()]
fn get_platform() -> vvm::Platform {
    if let Ok(s) = std::env::var(VVM_TARGET_PLATFORM) {
        s.parse().unwrap()
    } else {
        vvm::platform()
    }
...
pub static ALL_VYPER_VERSIONS : [semver::Version; {}] = [
    {}  ];
    "#,
        platform,
        version_idents.len(),
    ...
    let get_check_sum_fn = format!(
        r#"
/// Get the checksum of a vyper version's binary if it exists.
...
fn add_platform_const(writer: &mut build_const::ConstValueWriter, platform: vvm::Platform) {
    writer.add_raw(&format!(
        r#"
/// The `vvm::Platform` all constants were built for
...
fn main() {
    ...
    let releases = vvm::blocking_all_releases(platform).expect("Failed to fetch releases");
